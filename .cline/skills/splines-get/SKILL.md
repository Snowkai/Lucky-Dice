...
Serialize any Splines component on a GameObject using ReflectorNet. This is the generic escape hatch for fields not covered by the dedicated tools.

## Inputs

- `gameObjectRef` — the GameObject hosting the component (required).
- `componentRef` — optional. Resolves a specific component when the GameObject has more than one Splines component; otherwise the first component whose type lives in the `UnityEngine.Splines` namespace is used.
- `deepSerialization` — when `true`, recurses through nested objects; otherwise only top-level members.

## Behavior

Finds the target Splines component, serializes it via ReflectorNet, and returns the serialized member plus the resolved component type name. Read-only. Runs on the Unity main thread.

## How to Call

```bash
unity-mcp-cli run-tool splines-get --input '{
...
> ```bash
> unity-mcp-cli run-tool splines-get --input-file args.json
> ```
>
> Or pipe via stdin (recommended):
> ```bash
> unity-mcp-cli run-tool splines-get --input-file - <<'EOF'
> {"param": "value"}
> EOF
> ```
...
```json
{
  ...
  "properties": {
    "result": {
      "$ref": "#/$defs/com.IvanMurzak.Unity.MCP.Editor.API.Tool_Splines-SplinesGetResponse"
    }
  },
  "$defs": {
    "AIGD.GameObjectRef": {
    ...
    "com.IvanMurzak.Unity.MCP.Editor.API.Tool_Splines-SplinesGetResponse": {
      "type": "object",
      "properties": {
        "gameObjectRef": {
          "$ref": "#/$defs/AIGD.GameObjectRef",
